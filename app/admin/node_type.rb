ActiveAdmin.register NodeType do
  ...
  filter :category, as: :select, collection: proc { Category.all.each_with_object([]) { |r, memo| memo << [r.name, r.id]; } }
  filter :identifier
  filter :osm_key
  filter :osm_value
  filter :alt_osm_key
  ...
  form do |f|
    f.inputs do
      f.input :category, as: :select, collection: Category.all.each_with_object([]) { |r, memo| memo << [r.name, r.id]; }.sort
      f.input :identifier
      f.input :icon
    end
