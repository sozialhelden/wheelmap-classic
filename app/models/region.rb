class Region < ActiveRecord::Base
  ...
  def pois_of_children
    region_ids = Region.where(['regions.lft >= ? AND regions.rgt <= ?', lft, rgt]).select('id').map(&:id)
  ...
  def bounding_box
    bounding_box = RGeo::Cartesian::BoundingBox.BoundingBox.create_from_geometry(grenze)
  ...
  def self.from_wkt_file(wkt_file_name, parent)
    ...
    region_name = wkt_base_name.tr('_', ' ')
