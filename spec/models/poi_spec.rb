...
describe Poi do
  ...
  context 'other tag based attributes' do
    ...
    names.each do |attr|
      ...
      it "has virtual setters for #{attr}" do
        p = FactoryGirl.build(:poi)
        p.send "#{attr}=", 'horst'
        expect(p.tags[attr.to_s]).to eq('horst')
      end
    end
  end
  ...
  context 'json rendering' do
    context 'for osm' do
      REQUIRED_KEYS = [:id, :lat, :lon].freeze
  ...
  context '64 bit' do
    let :poi do
      Poi.new(osm_id: -4_294_967_934).tap do |p|
        p.lat = 52.0
        p.lon = 13.4
        p.version = 1
        p.type = 'restaurant'
