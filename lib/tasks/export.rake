namespace :export do
  ...
  task filter_region_nodes: :environment do
    ...
    csv_string = CSV.generate(force_quotes: true) do |csv_out|
      FCSV(STDIN, force_quotes: true, headers: true) do |csv_in|
        csv_out << %w(osm_id lat lon Rollstuhlstatus Kommentar name Typ Strasse Hausnummer PLZ Stadt Telefon URL)
        csv_in.each do |row|
  ...
  task for_stiftung_gesundheit: :environment do
    ...
    CSV.open(outfile, 'wb', force_quotes: true) do |csv|
      csv << %w(OSM_Id OSM_Type OSM_Name OSM_Rollstuhlstatus OSM_Latitude OSM_Longitude OSM_Strasse OSM_Hausnummer OSM_Stadt OSM_Plz)
  ...
  task for_tb_guide: :environment do
    headers = %w(Id Name Lat Lon Street Housenumber Postcode City)
  ...
  task for_streetspotr: :environment do
    ...
    CSV.open('streetspotr.csv', 'wb', force_quotes: true) do |csv|
      csv << %w(Id Name Lat Lon Street Housenumber Postcode City Wheelchair Type Category)
  ...
  task for_streetspotr_no_memorial: :environment do
    ...
    category_names  = %w(food shopping accommodation tourism)
